...
try {
    $stmt = $pdo->query("
        SELECT 
            c.user_id, c.firstname, c.lastname, c.phone_number,
            u.username
        FROM consumers c
...
function displayConsumersTable($pdo)
{
    ...
    // Count total rows
    $countQuery = "SELECT COUNT(*) FROM consumers c 
                   JOIN users u ON c.user_id = u.id 
                   WHERE c.firstname LIKE ? OR c.lastname LIKE ?";
    $stmt = $pdo->prepare($countQuery);
    $stmt->execute([$searchParam, $searchParam]);
    $totalConsumers = $stmt->fetchColumn();
    $totalPages = ceil($totalConsumers / $limit);

    // Fetch paginated farmers
    $query = "SELECT c.user_id, c.firstname, c.lastname, c.phone_number, u.username
              FROM consumers c 
              JOIN users u ON c.user_id = u.id
              WHERE c.firstname LIKE ? OR c.lastname LIKE ?
              LIMIT ? OFFSET ?";
    ...
    return compact('consumers', 'search', 'page', 'totalPages');
}

extract(displayConsumersTable($pdo));
?>
...
<body>
            ...
            <div class="sidebar-content">
                <div class="text-center">
                 ...
                 <nav class="nav flex-column">
                    ...
                    <a class="nav-link active" href="super_admin_consumers.php">
   ...
   <div class="d-flex flex-wrap justify-content-between align-items-start px-4 py-3">
    <h2 class="mb-0 me-3">Consumers</h2>
       ...
       <form method="GET" class="d-flex flex-grow-1 mt-2" style="max-width: 600px;">
        ...
        <?php if (!empty($search)): ?>
            <a 
                href="super_admin_consumers.php" 
                        ...
                        <?php foreach ($consumers as $consumer): ?>
                            <tr>
                                <td class="py-2 px-4 border-b" style="width: 20%;">
                                    <?= htmlspecialchars($consumer['firstname']) . ' ' . htmlspecialchars($consumer['lastname']) ?>
                                </td>
                                <td class="py-2 px-4 border-b" style="width: 25%;">
                                    <?= htmlspecialchars($consumer['username']) ?>
                                </td>
                                <td class="py-2 px-4 border-b" style="width: 20%;">
                                    <?= htmlspecialchars($consumer['phone_number']) ?>
                                </td>
                                <td class="py-2 px-4 border-b" style="width: 5%;">
                                    <button
                                        ...
                                        data-consumer-id="<?php echo $consumer['user_id']; ?>">
                                        Delete
                                    </button>
                                </td>
                            </tr>
                ...
                <div class="modal-content" style="background-color: #1a1a1a; color: #fff; border-radius: 10px;">
                    ...
                    <div class="modal-footer">
                        <form id="deleteForm" method="POST" action="super_admin_controller.php">
                            <input type="hidden" name="action" value="delete_consumer">
                            <input type="hidden" name="delete_consumer_id" id="deleteConsumerId">
        ...
        <script>
            document.querySelectorAll('.btn-delete').forEach(button => {
                button.addEventListener('click', function() {
                    const consumerId = this.getAttribute('data-consumer-id');
                    console.log(consumerId);

                    document.getElementById('deleteConsumerId').value = consumerId;
                });
            });
        </script>
