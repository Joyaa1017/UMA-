...
session_start();
require_once 'db.php';
// require_once 'super_admin_controller.php';
// displayFarmersTable($pdo);

try {
    $stmt = $pdo->query("
        SELECT 
            f.user_id, f.firstname, f.lastname, f.phone_number, f.farmname, 
            u.username
        FROM farmers f
    ...
    die("Error fetching farmers: " . $e->getMessage());
}

function displayFarmersTable($pdo)
{
    ...
    // Count total rows
    $countQuery = "SELECT COUNT(*) FROM farmers f 
                   JOIN users u ON f.user_id = u.id 
                   WHERE f.firstname LIKE ? OR f.lastname LIKE ? OR f.farmname LIKE ?";
    ...
    $query = "SELECT f.user_id, f.firstname, f.lastname, f.phone_number, f.farmname, u.username
              FROM farmers f 
              JOIN users u ON f.user_id = u.id
              WHERE f.firstname LIKE ? OR f.lastname LIKE ? OR f.farmname LIKE ?
              LIMIT ? OFFSET ?";
    ...
    return compact('farmers', 'search', 'page', 'totalPages');
}

extract(displayFarmersTable($pdo));
?>
...
<body>
            ...
            <div class="sidebar-content">
                <div class="text-center">
                 ...
                 <nav class="nav flex-column">
                    ...
                    <a class="nav-link active" href="super_admin_farmers.php">
   ...
   <div class="d-flex flex-wrap justify-content-between align-items-start px-4 py-3">
    ...
    <div class="bg-custom-dark rounded-lg shadow-lg">
             ...
             <table class="min-w-full table-fixed bg-green">
    <thead style="background-color:rgb(255, 255, 255); color:#000000;">
        <tr>
            <th class="py-2 px-4 border-b text-left" style="width: 20%;">Name</th>
            <th class="py-2 px-4 border-b text-left" style="width: 25%;">Email</th>
            <th class="py-2 px-4 border-b text-left" style="width: 20%;">Phone Number</th>
            <th class="py-2 px-4 border-b text-left" style="width: 25%;">Farm Name</th>
            <th class="py-2 px-4 border-b text-left" style="width: 5%;">Action</th>
        </tr>
    </thead>
    <tbody id="farmersTable">
            ...
            <?php foreach ($farmers as $farmer): ?>
                <tr>
                    <td class="py-2 px-4 border-b" style="width: 20%;">
                        <?= htmlspecialchars($farmer['firstname']) . ' ' . htmlspecialchars($farmer['lastname']) ?>
                    </td>
                    <td class="py-2 px-4 border-b" style="width: 25%;">
                        <?= htmlspecialchars($farmer['username']) ?>
                    </td>
                    <td class="py-2 px-4 border-b" style="width: 20%;">
                        <?= htmlspecialchars($farmer['phone_number']) ?>
                    </td>
                    <td class="py-2 px-4 border-b" style="width: 25%;">
                        <?= htmlspecialchars($farmer['farmname']) ?>
                    </td>
                    <td class="py-2 px-4 border-b text-center" style="width: 5%;">
                        <button
                            type="button"
        ...
        <?php else: ?>
            <tr>
                <td colspan="5" class="text-center py-3">No farmers found.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
                ...
                <div class="modal-content" style="background-color: #1a1a1a; color: #fff; border-radius: 10px;">
                    ...
                    <div class="modal-footer">
                        <form id="deleteForm" method="POST" action="super_admin_controller.php">
                            ...
                            <input type="hidden" name="delete_farmer_id" id="deleteFarmerId">
