...
$cart_item_ids = array_filter($_POST['selected_cart_items'] ?? [], fn($id) => is_numeric($id));

if (empty($cart_item_ids)) {
    die("No cart items selected.");
}

// Fetch consumer and user details
...
// Fetch selected cart item details
$placeholders = implode(',', array_fill(0, count($cart_item_ids), '?'));
$cartStmt = $pdo->prepare("SELECT * FROM carts WHERE id IN ($placeholders) AND user_id = ?");
$params = array_merge($cart_item_ids, [$user_id]);
$cartStmt->execute($params);
$selectedCarts = $cartStmt->fetchAll();

$quantities = $_POST['quantities'] ?? []; // optional
...
foreach ($selectedCarts as $cartItem) {
    ...
    if ($product) {
        $quantity = $quantities[$cartItem['id']] ?? $cartItem['product_quantity'];
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <div class="order-summary">
                    <form action="consumer_cart_transaction.php" method="POST">
                        <?php foreach ($cartProducts as $item): ?>
                            <input type="hidden" name="cart_item_ids[]" value="<?= htmlspecialchars($item['product_id']) ?>">
                            <input type="hidden" name="cart_item_quantities[]" value="<?= htmlspecialchars($item['quantity']) ?>">
                            <div class="item">
                                <div>
                                ...
                                <img src="img/<?= htmlspecialchars($item['product_image']) ?>" alt="Image" />
                            </div>
                        <?php endforeach; ?>
